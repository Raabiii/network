...
public class Farming {
    ...
    public void addAmount(Player p, int amount){
        ...
        if (currentAmount.get(p.getUniqueId()) >= LevelRequirements.level(level.get(p.getUniqueId()))){
            ...
            p.sendMessage("§3You have reached Farming level: " + level.get(p.getUniqueId()) + " and got§6 " +  LevelRequirements.levelMoney(level.get(p.getUniqueId())) + "€");
        ...
        sendActionBar(p, "§3+ " + amount + " Farming: " + getCurrentAmount(p) + "/" + LevelRequirements.level(getCurrentLevel(p)) + " (" + (getCurrentAmount(p)*100)/LevelRequirements.level(getCurrentLevel(p)) + "%)");
    }
    ...
    public void saveSkill(){
        File fileLevel = new File("skill/farmingLevel.yml");
        File fileAmount = new File("skill/farmingAmount.yml");
