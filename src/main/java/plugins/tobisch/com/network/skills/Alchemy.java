...
public class Alchemy {
    ...
    public void addAmount(Player p, int amount){
        ...
        if (currentAmount.get(p.getUniqueId()) >= LevelRequirements.level(level.get(p.getUniqueId()))){
            ...
            p.sendMessage("§3You have reached Alchemy level: " + level.get(p.getUniqueId()) + " and got§6 " +  LevelRequirements.levelMoney(level.get(p.getUniqueId())) + "€");
        ...
        sendActionBar(p, "§3+ " + amount + " Alchemy: " + getCurrentAmount(p) + "/" + LevelRequirements.level(getCurrentLevel(p)) + " (" + (getCurrentAmount(p)*100)/LevelRequirements.level(getCurrentLevel(p)) + "%)");
    }
    ...
    public void saveSkill(){
        File fileLevel = new File("skill/alchemyLevel.yml");
        File fileAmount = new File("skill/alchemyAmount.yml");
