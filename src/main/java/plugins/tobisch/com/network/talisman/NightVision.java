...
public class NightVision implements Talisman{
    ...
    public ItemStack createTalisman(){
        ...
        skullMeta.setOwningPlayer(Bukkit.getOfflinePlayer("Bogerang")); // Set the skull's owner so it will adapt the skin of the provided username (case sensitive).
        skullMeta.setDisplayName("§5§kr§r§5 Night vision Talisman §kf§r");
    ...
    public ItemStack createRing(){
        ...
        assert skullMeta != null;
        skullMeta.setOwningPlayer(Bukkit.getOfflinePlayer("Bogerang")); // Set the skull's owner so it will adapt the skin of the provided username (case sensitive).
        skullMeta.setDisplayName("§6§kr§r§6 Night vision Ring §kf§r");
        skullMeta.setLore(lore);
        lifeSaver.setItemMeta(skullMeta);
    ...
    public ItemStack createArtifact(){
        ...
        assert skullMeta != null;
        skullMeta.setOwningPlayer(Bukkit.getOfflinePlayer("Bogerang")); // Set the skull's owner so it will adapt the skin of the provided username (case sensitive).
        skullMeta.setDisplayName("§d§kr§r§d Night vision Artifact §kf§r");
        skullMeta.setLore(lore);
        lifeSaver.setItemMeta(skullMeta);
    ...
    public int compare(ItemStack item1){
        ...
        if (meta1.hasDisplayName() && meta1.getDisplayName().contains("Night vision Talisman") ){
            return 1;
        } else if (meta1.hasDisplayName() && meta1.getDisplayName().contains("Night vision Ring") ) {
            return 2;
        } else if (meta1.hasDisplayName() && meta1.getDisplayName().contains("Night vision Artifact") )
            return 3;
