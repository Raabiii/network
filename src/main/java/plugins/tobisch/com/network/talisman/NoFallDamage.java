...
public class NoFallDamage implements Talisman{
    ...
    public ItemStack createTalisman(){
        ...
        skullMeta.setOwningPlayer(Bukkit.getOfflinePlayer("DragonFury4250")); // Set the skull's owner so it will adapt the skin of the provided username (case sensitive).
        skullMeta.setDisplayName("§5§kr§r§5 No fall damage Talisman §kf§r");
    ...
    public ItemStack createRing(){
        ...
        assert skullMeta != null;
        skullMeta.setOwningPlayer(Bukkit.getOfflinePlayer("DragonFury4250")); // Set the skull's owner so it will adapt the skin of the provided username (case sensitive).
        skullMeta.setDisplayName("§6§kr§r§6 No fall damage Ring §kf§r");
        skullMeta.setLore(lore);
        lifeSaver.setItemMeta(skullMeta);
    ...
    public ItemStack createArtifact(){
        ...
        assert skullMeta != null;
        skullMeta.setOwningPlayer(Bukkit.getOfflinePlayer("DragonFury4250")); // Set the skull's owner so it will adapt the skin of the provided username (case sensitive).
        skullMeta.setDisplayName("§d§kr§r§d No fall damage Artifact §kf§r");
        skullMeta.setLore(lore);
        lifeSaver.setItemMeta(skullMeta);
    ...
    public int compare(ItemStack item1){
        ...
        if (meta1.hasDisplayName() && meta1.getDisplayName().contains("No fall damage Talisman") ){
            return 1;
        } else if (meta1.hasDisplayName() && meta1.getDisplayName().contains("No fall damage Ring") ) {
            return 2;
        } else if (meta1.hasDisplayName() && meta1.getDisplayName().contains("No fall damage Artifact") )
            return 3;
