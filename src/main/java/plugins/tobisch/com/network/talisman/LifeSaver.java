...
public class LifeSaver implements Talisman{
    ...
    public ItemStack createTalisman(){
        ...
        skullMeta.setOwningPlayer(Bukkit.getOfflinePlayer("Technoblade")); // Set the skull's owner so it will adapt the skin of the provided username (case sensitive).
        skullMeta.setDisplayName("§5§kr§r§5 Life Saver Talisman §kf§r");
    ...
    public ItemStack createRing(){
        ...
        assert skullMeta != null;
        skullMeta.setOwningPlayer(Bukkit.getOfflinePlayer("Technoblade")); // Set the skull's owner so it will adapt the skin of the provided username (case sensitive).
        skullMeta.setDisplayName("§6§kr§r§6 Life Saver Ring §kf§r");
        skullMeta.setLore(lore);
        lifeSaver.setItemMeta(skullMeta);
    ...
    public ItemStack createArtifact(){
        ...
        assert skullMeta != null;
        skullMeta.setOwningPlayer(Bukkit.getOfflinePlayer("Technoblade")); // Set the skull's owner so it will adapt the skin of the provided username (case sensitive).
        skullMeta.setDisplayName("§d§kr§r§d Life Saver Artifact §kf§r");
        skullMeta.setLore(lore);
        lifeSaver.setItemMeta(skullMeta);
    ...
    public int compare(ItemStack item1){
        ...
        if (meta1.hasDisplayName() && meta1.getDisplayName().contains("Life Saver Talisman") ){
            return 1;
        } else if (meta1.hasDisplayName() && meta1.getDisplayName().contains("Life Saver Ring") ) {
            return 2;
        } else if (meta1.hasDisplayName() && meta1.getDisplayName().contains("Life Saver Artifact") )
            return 3;
